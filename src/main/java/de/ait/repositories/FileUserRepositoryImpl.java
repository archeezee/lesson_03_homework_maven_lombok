...
public class FileUserRepositoryImpl implements UserCrudRepository {
    ...
    public List<User> findAll() {
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            return reader.lines().map(s -> s.split(";")).map(uArr -> new User(Long.parseLong(uArr[0]), uArr[1], uArr[2])).toList();
        } catch (IOException e) {
    ...
    public List<User> findBuNameStartingWith(String prefix) {
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            return reader.lines().map(s -> s.split(";")).filter(u -> u[1].startsWith(prefix)).map(uArr -> new User(Long.parseLong(uArr[0]), uArr[1], uArr[2])).toList();
        } catch (IOException e) {
            System.out.println("File read error");
            return Collections.EMPTY_LIST;
        }
    ...
    public User findByID(Long id) {
        return findAll().stream().filter(u -> u.getId() == id.longValue()).findFirst().orElse(null);
    }
